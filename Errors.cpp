...
static ErrorStrings Errors[] =
{
    ...
    {ERR_NO_SMTP_SPOOL_FILES, "No SMTP spool files ready to process"},
    ...
    {ERR_SPOOL_FILE_NOT_READY, "Spool file not ready"},
    ...
    {ERR_INVALID_QUEUE_PATH, "Invalid queue path"},
    {ERR_QUEUE_ARRAY_FULL, "Queue array is full"},
    {ERR_QUEUE_ENTRY_NOT_FOUND, "Queue entry not found"},
...
char           *ErrGetErrorStringInfo(int iError)
{
    ...
    if (pEV->pszInfo[iErrIndex] != NULL)
        sprintf(pszErrorInfo,
                "ErrCode   = %d\n"
                "ErrString = %s\n"
                "ErrInfo   = %s", iError, Errors[iErrIndex].pszError, pEV->pszInfo[iErrIndex]);
    else
        sprintf(pszErrorInfo,
                "ErrCode   = %d\n"
