...
Example. Suppose a filter modified the message and hence needs to return B<100> as return code.
Suppose also that a filter wants to block the filter selection list processing by
specifying a flags value of B<8>, the value to be returned will be :

B<FRC> = B<100> + B<8> = B<108>
...
(AV scanning, content filter, message rewriting, etc) by returning 'B<100>'.
The filter 'B<MUST>' return 'B<100>' in case it modifies the message.
...
on the real message, and a badly reformatted file will lead to message loss.
