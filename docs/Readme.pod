...
XMail sources compile under GNU/Linux, FreeBSD, OpenBSD, Solaris and NT/2K.
...
As soon as the project reaches a higher maturity I plan to supply a configure script.
...
Setup the file 'B<smtprelay.tab>' (and/or 'B<smtp.ipmap.tab>' if necessary) to restrict mail
relaying of your server. 'B<THIS IS IMPORTANT!>'
...
Now if the user 'foo' replies to this message, it replies to 'dlibenzimaticad.it',
...
accounts is performed to avoid duplicating informations with 'B<SMTPAUTH.TAB>'.
...
from 'B<MAILPROC.TAB>' file processing.
...
XMail provides a way to handle this task by if th 'B<userauth/smtp>'
...
redirects the message to 'B<userE<64>target-domain.org>'.
...
When a mail from 'B<USERE<64>DOMAIN>' is received by the server, the domain 'B<DOMAIN>'
is to be
...
These 'filters' may examine and/or
modify messages and inform XMail of their actions with a return value.
...
If this filters returns 'B<97, 98 or 99>' the message is rejected and
is stopped in its travel.
If the filter modifies the message it must return 'B<100>'.

When a message is received by the SMTP server for user 'B<fooE<64>xyzw.aiai.abc>' XMail
searches inside the 'B<filters>' subdirectory to find a file named (user processing):
...
The 'B<.tab>' file offers a way to specify a default mail filtering.

If none of the above files are found the message continues its travel, otherwise
the file is processed by submitting the message to all filters stored in the file.
...
the message file path (the external command may modify
the file if it returns 'B<100>' as command exit value.)
...
returns its processing result. If it returns 'B<99>' the message is rejected and a notification
message is sent to the sender. By returning 'B<98>' the message is rejected
without notification.
While returning 'B<97>' the message is rejected without notification and without being frozen
(a 'B<98>' response could lead to a frozen message if the L<"SERVER.TAB"> configuration enables this).
If all filters return values different from 'B<99, 98 and 97>' the message continues its trip.
The filter command may also modify the file
(AV scanning, content filter, message rewriting, etc)
by returning 'B<100>'.
If the filter changes the message file it 'B<MUST>' keep the message structure and
it 'B<MUST>' terminate all line with <CR><LF>.
...
of a headers section and, after the first empty line, the message body.
...
If you have enabled logging, remember to setup the 'B<-Mr ndays>' option depending on the
