...
{
	unsigned long ulMBSize = 0, ulProbeSize = (pulAvailSpace != NULL) ? *pulAvailSpace: 0;
	...
	if (pszMaxMBSize != NULL) {
		...
		if (ulMaxMBSize && (ulMBSize + ulProbeSize >= ulMaxMBSize)) {
...
int UPopSessionSendMsg(POP3_HANDLE hPOPSession, int iMsgIndex, BSOCK_HANDLE hBSock)
{
	...
	if (pPOPMD->ulMsgSize > 0 &&
	    BSckSendFile(hBSock, szMsgFilePath, 0, (unsigned long) -1,
			 pPOPSD->iTimeout) < 0)
		return ErrGetErrorCode();
